...
public class Alicia_moof2 : MonoBehaviour
{
    ...
    private int buttonbetween = 20 + 5;

    private int screenwidth = Screen.width;
    //private int screenheight = Screen.height;

    //private float RsliderVal = 0.0f;
    //private float GsliderVal = 0.0f;
    //private float BsliderVal = 0.0f;
    ...
    public float mouth_a = 0;
    public float mouth_i = 0;
    public float mouth_u = 0;
    public float mouth_e = 0;
    public float mouth_o = 0;
    public float mouth_a2 = 0;
    public float mouth_n = 0;
    public float mouth_triangle = 0;
    public float mouth_lambda = 0;
    public float mouth_square = 0;
    public float mouth_wa = 0;
    public float mouth_wa2 = 0;
    public float mouth_shock = 0;
    public float mouth_angry = 0;
    public float mouth_smile = 0;
    public float mouth_spear = 0;
    public float mouth_spear2 = 0;
    public float mouth_cornerUP = 0;
    public float mouth_cornerDown = 0;
    public float mouth_cornerSpear = 0;
    public float mouth_noTeethUP = 0;
    public float mouth_noTeethDown = 0;
    public float mouth_Tu = 0;
    public float mouth_be = 0;
    public float eye_blink = 0;
    public float eye_smile = 0;
    public float eye_winkL = 0;
    public float eye_winkR = 0;
    public float eye_winkL2 = 0;
    public float eye_winkR2 = 0;
    public float eye_Calm = 0;
    public float eye_shock = 0;
    public float eye_surprised = 0;
    public float eye_TT = 0;
    public float eye_serious = 0;
    public float eye_hacyu = 0;
    public float eyeblow_serious = 0;
    public float eyeblow_trouble = 0;
    public float eyeblow_smile = 0;
    public float eyeblow_angry = 0;
    public float eyeblow_up = 0;
    public float eyeblow_down = 0;
    public float eyeblow_gather = 0;
    public float face_view = 0;
    public float face_rignt = 0;
    public float face_left = 0;
    public float tongue = 0;
    public float mouth_mortifying = 0;
    public float eye_up = 0;
    //other
    public float other_shy = 0;
    public float eye_h2 = 0;
    public float eye_h3 = 0;
    public float other_shocked = 0;
    public float other_tear = 0;
    //other02
    public float other_shy2 = 0;

    private bool[] mouth_Button = new bool[25];
    private bool[] eye_Button = new bool[15];
    private bool[] eyeblow_Button = new bool[7];
    private bool[] face_Button = new bool[4];
    private bool[] other_Button = new bool[4];
    ...
    void Update()
    {
        ...
        //mouth
        if (mouth_Button[0])
            mouth_a = Mathf.Min(mouth_a + 10, 100);
        else
            mouth_a = Mathf.Max(0, mouth_a - 10);

        if (mouth_Button[1])
            mouth_i = Mathf.Min(mouth_i + 10, 100);
        else
            mouth_i = Mathf.Max(0, mouth_i - 10);

        if (mouth_Button[2])
            mouth_u = Mathf.Min(mouth_u + 10, 100);
        else
            mouth_u = Mathf.Max(0, mouth_u - 10);

        if (mouth_Button[3])
            mouth_e = Mathf.Min(mouth_e + 10, 100);
        else
            mouth_e = Mathf.Max(0, mouth_e - 10);

        if (mouth_Button[4])
            mouth_o = Mathf.Min(mouth_o + 10, 100);
        else
            mouth_o = Mathf.Max(0, mouth_o - 10);

        if (mouth_Button[5])
            mouth_a2 = Mathf.Min(mouth_a2 + 10, 100);
        else
            mouth_a2 = Mathf.Max(0, mouth_a2 - 10);

        if (mouth_Button[6])
            mouth_n = Mathf.Min(mouth_n + 10, 100);
        else
            mouth_n = Mathf.Max(0, mouth_n - 10);

        if (mouth_Button[7])
            mouth_triangle = Mathf.Min(mouth_triangle + 10, 100);
        else
            mouth_triangle = Mathf.Max(0, mouth_triangle - 10);

        if (mouth_Button[8])
            mouth_lambda = Mathf.Min(mouth_lambda + 10, 100);
        else
            mouth_lambda = Mathf.Max(0, mouth_lambda - 10);

        if (mouth_Button[9])
            mouth_square = Mathf.Min(mouth_square + 10, 100);
        else
            mouth_square = Mathf.Max(0, mouth_square - 10);

        if (mouth_Button[10])
            mouth_wa = Mathf.Min(mouth_wa + 10, 100);
        else
            mouth_wa = Mathf.Max(0, mouth_wa - 10);

        if (mouth_Button[11])
            mouth_wa2 = Mathf.Min(mouth_wa2 + 10, 100);
        else
            mouth_wa2 = Mathf.Max(0, mouth_wa2 - 10);

        if (mouth_Button[12])
            mouth_shock = Mathf.Min(mouth_shock + 10, 100);
        else
            mouth_shock = Mathf.Max(0, mouth_shock - 10);

        if (mouth_Button[13])
            mouth_angry = Mathf.Min(mouth_angry + 10, 100);
        else
            mouth_angry = Mathf.Max(0, mouth_angry - 10);

        if (mouth_Button[14])
            mouth_smile = Mathf.Min(mouth_smile + 10, 100);
        else
            mouth_smile = Mathf.Max(0, mouth_smile - 10);

        if (mouth_Button[15])
            mouth_spear = Mathf.Min(mouth_spear + 10, 100);
        else
            mouth_spear = Mathf.Max(0, mouth_spear - 10);

        if (mouth_Button[16])
            mouth_spear2 = Mathf.Min(mouth_spear2 + 10, 100);
        else
            mouth_spear2 = Mathf.Max(0, mouth_spear2 - 10);

        if (mouth_Button[17])
            mouth_cornerUP = Mathf.Min(mouth_cornerUP + 10, 100);
        else
            mouth_cornerUP = Mathf.Max(0, mouth_cornerUP - 10);

        if (mouth_Button[18])
            mouth_cornerDown = Mathf.Min(mouth_cornerDown + 10, 100);
        else
            mouth_cornerDown = Mathf.Max(0, mouth_cornerDown - 10);

        if (mouth_Button[19])
            mouth_cornerSpear = Mathf.Min(mouth_cornerSpear + 10, 100);
        else
            mouth_cornerSpear = Mathf.Max(0, mouth_cornerSpear - 10);

        if (mouth_Button[20])
            mouth_noTeethUP = Mathf.Min(mouth_noTeethUP + 10, 100);
        else
            mouth_noTeethUP = Mathf.Max(0, mouth_noTeethUP - 10);

        if (mouth_Button[21])
            mouth_noTeethDown = Mathf.Min(mouth_noTeethDown + 10, 100);
        else
            mouth_noTeethDown = Mathf.Max(0, mouth_noTeethDown - 10);

        if (mouth_Button[22])
            mouth_Tu = Mathf.Min(mouth_Tu + 10, 100);
        else
            mouth_Tu = Mathf.Max(0, mouth_Tu - 10);

        if (mouth_Button[23])
            mouth_be = Mathf.Min(mouth_be + 10, 100);
        else
            mouth_be = Mathf.Max(0, mouth_be - 10);
        ...
        //eyeblow
        if (eyeblow_Button[0])
            eyeblow_serious = Mathf.Min(eyeblow_serious + 10, 100);
        else
            eyeblow_serious = Mathf.Max(0, eyeblow_serious - 10);

        if (eyeblow_Button[1])
            eyeblow_trouble = Mathf.Min(eyeblow_trouble + 10, 100);
        else
            eyeblow_trouble = Mathf.Max(0, eyeblow_trouble - 10);

        if (eyeblow_Button[2])
            eyeblow_smile = Mathf.Min(eyeblow_smile + 10, 100);
        else
            eyeblow_smile = Mathf.Max(0, eyeblow_smile - 10);

        if (eyeblow_Button[3])
            eyeblow_angry = Mathf.Min(eyeblow_angry + 10, 100);
        else
            eyeblow_angry = Mathf.Max(0, eyeblow_angry - 10);

        if (eyeblow_Button[4])
            eyeblow_up = Mathf.Min(eyeblow_up + 10, 100);
        else
            eyeblow_up = Mathf.Max(0, eyeblow_up - 10);

        if (eyeblow_Button[5])
            eyeblow_down = Mathf.Min(eyeblow_down + 10, 100);
        else
            eyeblow_down = Mathf.Max(0, eyeblow_down - 10);

        if (eyeblow_Button[6])
            eyeblow_gather = Mathf.Min(eyeblow_gather + 10, 100);
        else
            eyeblow_gather = Mathf.Max(0, eyeblow_gather - 10);

        if (face_Button[0])
            face_view = Mathf.Min(face_view + 10, 100);
        else
            face_view = Mathf.Max(0, face_view - 10);

        if (face_Button[1])
            face_rignt = Mathf.Min(face_rignt + 10, 100);
        else
            face_rignt = Mathf.Max(0, face_rignt - 10);

        if (face_Button[2])
            face_left = Mathf.Min(face_left + 10, 100);
        else
            face_left = Mathf.Max(0, face_left - 10);

        if (face_Button[3])
            tongue = Mathf.Min(tongue + 10, 100);
        else
            tongue = Mathf.Max(0, tongue - 10);

        if (mouth_Button[24])
            mouth_mortifying = Mathf.Min(mouth_mortifying + 10, 100);
        else
            mouth_mortifying = Mathf.Max(0, mouth_mortifying - 10);
        ...
        if (other_Button[0])
            other_shy = Mathf.Min(other_shy + 10, 100);
        else
            other_shy = Mathf.Max(0, other_shy - 10);
        ...
        if (other_Button[2])
            other_shocked = Mathf.Min(other_shocked + 10, 100);
        else
            other_shocked = Mathf.Max(0, other_shocked - 10);

        if (other_Button[3])
            other_tear = Mathf.Min(other_tear + 10, 100);
        else
            other_tear = Mathf.Max(0, other_tear - 10);

        if (other_Button[1])
            other_shy2 = Mathf.Min(other_shy2 + 10, 100);
        else
            other_shy2 = Mathf.Max(0, other_shy2 - 10);
